...
class CharRNNExperiment :
    ...
    def run(self, tdf, vdf):
        ...
        for language in C.LANGUAGE_TO_ISO_MAPPING.keys():
            name = 'char-rnn'
            X_train = tdf[language]['char_embedding'].values[:]
            y_train = numpy.array(tdf[language]['char_label'].values[:])
            X_valid = vdf[language]['char_embedding'].values[:]
            y_valid = numpy.array(tdf[language]['char_label'].values[:])
            ...
            RNN = CharacterRNN(language=language, max_features=self.embedding_size, embedding_size=self.embedding_size,\
                filter_length=self.filter_length, nb_filter=self.embedding_size, pool_length=self.filter_length,\
                    lstm_output_size=self.lstm_output_size, epochs=30)
    ...
    def _save_eval_data(self, eval_df, tag, Masterdir = os.getcwd(), Experimentdir = C.EXPERIMENT_DIR):
        ...
        data_path = os.path.join(base_path, tag + '.tsv')
